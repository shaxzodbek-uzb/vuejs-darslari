<template>
  <div class="partners">
    <div class="container">
      <div class="partners__content row">
        <div class="partners__title col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
          Наши
          <span class="ontheground">партнёры</span>
        </div>
        <div
          class="partners__txt col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6"
        >Компания Frutata — новичок на рынке Узбекистана, однако несмотря на свою короткую историю мы являемся одним.</div>
      </div>
      <div class="row" id="yes">
        <div id="cherry" class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xl-3">
          <div class="cardo">
            <img src="/logo/frulogo.svg" alt />
          </div>
        </div>
        <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xl-3" id="cherry">
          <div class="cardo">
            <img src="/logo/fruitdorlogo.png" alt />
          </div>
        </div>
        <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xl-3" id="cherry">
          <div class="cardo">
            <img src="/logo/organicslogo.png" alt />
          </div>
        </div>
        <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xl-3" id="cherry">
          <div class="cardo">
            <img src="/logo/korzinkalogo.png" alt />
          </div>
        </div>
      </div>
    </div>
  </div>
</template>